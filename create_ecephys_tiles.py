...
def create_ecephys_tiles(
    *,
    ...
    while segment_size % (downsampling_base**num_levels) != 0:
        segment_size *= downsampling_base
    # Let's process at least 1 second at a time, so we ensure that
    while segment_size / R.get_sampling_frequency() < 1.0:
        segment_size *= downsampling_base
    print(f'Using segment size {segment_size} samples ({segment_size / R.get_sampling_frequency():.3f} sec)')
